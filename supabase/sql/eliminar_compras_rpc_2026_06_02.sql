...
begin
  ...
  loop
    ...
    if v_stock_final < 0 then
      raise exception 'No se puede eliminar la compra porque % quedaría con stock negativo.', v_producto.nombre;
    end if;
    ...
    values (
      ...
      v_linea.cantidad,
      current_date,
      'Eliminación compra factura ' || v_compra.numero_factura || '. Se descuenta la entrada registrada.',
      v_stock_final,
      v_usuario_id
    )
